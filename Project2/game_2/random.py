    ...
    for i in range(15):
        for j in range(15):
            ...
            for k in dir:
                if k == (0, 0):
                    continue
                if(i + k[0] >= 0 and i + k[0] < 15 and j + k[1] >= 0 and j + k[1] < 15 and mapStat[i + k[0]][j + k[1]] == -1):
                    valid = 1
                    break
...
def GetStep(playerID, mapStat, sheepStat, agent):
        ...
        if mapStat[i][j] == playerID and sheepStat[i][j] > 1:
            ...
            for k in dir:
                if k == (0,0):
                    continue
                x, y = i + k[0], j + k[1]
                if 0 <= x < 15 and 0 <= y < 15 and mapStat[x][y] == 0:
                    valid = 1
                    break
            if valid:
    ...
    tp_move = list(move)
    while(0 <= tp_move[0] + direction[0] < 15 and 0 <= tp_move[1] + direction[1] < 15 and mapStat[tp_move[0] + direction[0]][tp_move[1] + direction[1]] == 0):
